...
async function login(init){const u=await verify(init),q=await admin('/rest/v1/telegram_accounts?select=user_id&telegram_user_id=eq.'+u.id),email='telegram-'+u.id+'@telegram.kartochka.invalid',password=crypto.randomUUID()+'-'+crypto.randomUUID();let id=q[0]?.user_id;if(!id){const x=await admin('/auth/v1/admin/users',{method:'POST',body:JSON.stringify({email,password,email_confirm:true,user_metadata:{auth_source:'telegram',telegram_id:u.id,telegram_username:u.username||null,telegram_first_name:u.first_name||null}})});id=x.user?.id;if(!id)throw Error('Unable to create Telegram account');await admin('/rest/v1/telegram_accounts',{method:'POST',headers:{Prefer:'resolution=ignore-duplicates,return=minimal'},body:JSON.stringify({telegram_user_id:u.id,user_id:id,telegram_username:u.username||null,telegram_first_name:u.first_name||null})})}await admin('/auth/v1/admin/users/'+id,{method:'PUT',body:JSON.stringify({password,user_metadata:{auth_source:'telegram',telegram_id:u.id,telegram_username:u.username||null,telegram_first_name:u.first_name||null}})});const session=await admin('/auth/v1/token?grant_type=password',{method:'POST',body:JSON.stringify({email,password})});if(!session.access_token||!session.user?.id)throw Error('Unable to create Telegram session');return session}
